...
| 7.1 - 10.0     | [Latest](https://github.com/Dominic-DallOsto/zotero-reading-list/releases/latest)     |
...
| Option                                                         | Description                                                                                                                                                                                                                                                           |
| -------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Enable Keyboard Shortcuts                                      | Enable using keyboard shortcuts to set an item's read status. By default this is Alt+NUM but can be customised. Note: the default keyboard shortcuts will disable Zotero's builtin shortcut for changing the column used for sorting in the item pane (also Alt+NUM). |
...
| Custom Read Statuses and Icons and Keyboard shortcuts[^1]      | Choose custom Read Status names, icons, and keyboard shortcuts. Note: if you delete a read status, it will remain assigned to any items - you'll need to delete / change their statuses manually.                                                                     |
...
| Tag and Read Status Synchronisation[^1]                        | If enabled, matching tags will automatically be applied to items when you change their read status, so you can also filter by and change item read statuses with the corresponding tags when using the Zotero app (that doesn't support plugins)                      |
